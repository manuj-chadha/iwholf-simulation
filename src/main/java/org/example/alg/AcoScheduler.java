...
public class AcoScheduler extends MetaheuristicScheduler {
    public AcoScheduler(Fitness fit){ super(fit); }
        ...
        for(int it=0; it<iterations; it++){
            ...
            for(int a=0; a<ants; a++){
                ...
                for(int i=0; i<n; i++){
                    ...
                    int choice=0;
                    for(int vm=0; vm<k; vm++){ acc+=prob[vm]; if(r<=acc){ choice=vm; break; } }
                    s[i]=choice;
                }
                sols.add(s);
            ...
            // evaporation
            for(int i=0;i<n;i++) for(int vm=0;vm<k;vm++) tau[i][vm] *= (1.0 - rho);

            // reinforce best in this iteration
            int ib = argmin(fs);
            int[] sb = sols.get(ib);
            double fb = fs.get(ib);
            for(int i=0;i<n;i++) tau[i][sb[i]] += 1.0/(1.0+fb);

            if(fb < bestF){ bestF=fb; best=sb.clone(); }
        }
        long ms = System.currentTimeMillis()-t0;
        return new Result(best, bestF, ms);
    }

    private static int argmin(List<Double> a){ int idx=0; for(int i=1;i<a.size();i++) if(a.get(i)<a.get(idx)) idx=i; return idx; }
}